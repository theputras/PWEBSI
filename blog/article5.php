...
$xmlFile = './assets/article/article5.xml';
...
function generateSafeAltText($title) {
    // Strip any HTML tags and truncate if too long
    $safeAlt = strip_tags($title);
    $safeAlt = htmlspecialchars($safeAlt, ENT_QUOTES, 'UTF-8');
    return strlen($safeAlt) > 100 ? substr($safeAlt, 0, 100) . '...' : $safeAlt;
}
...
foreach ($sections as $section) {
    ...
    if (isset($section->image) && !empty($section->image['src'])) {
        $defaultAltText = generateSafeAltText($title);
        $imageAlt = !empty($section->image['alt']) ? htmlspecialchars($section->image['alt'], ENT_QUOTES, 'UTF-8') : $defaultAltText;

        echo "<figure class='mb-5'>";
        echo "<img src='" . $section->image['src'] . "' alt='" . $imageAlt . "' class='img-fluid mb-3 d-block mx-auto' style='border-radius: 10px;'>";
...
foreach ($subsections as $subsection) {
    ...
    // Check if image exists before rendering
    if (isset($subsection->image) && !empty($subsection->image['src'])) {
        $defaultAltText = generateSafeAltText($title);
        $imageAlt = !empty($subsection->image['alt']) ? htmlspecialchars($subsection->image['alt'], ENT_QUOTES, 'UTF-8') : $defaultAltText;

        echo "<figure>";
        echo "<img src='" . $subsection->image['src'] . "' alt='" . $imageAlt . "' class='img-fluid mb-5 d-block mx-auto' style='border-radius: 10px;'>";
